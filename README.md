...
2. 修改以下配置项：
   ...
   - **管理员密码**: 登录系统的密码
   - **Telegram配置**: 可选的通知功能
   - **固定入口映射**: 为 Mihomo/Clash.Meta listeners 绑定固定端口、节点、用户名和密码

### 固定入口映射

1. 先在左侧选择一个分组，并确保该分组已有启用订阅
2. 打开“系统配置”，点击“刷新节点”加载当前分组解析后的节点候选
3. 可手动新增映射，也可填写数量和端口范围后点击“随机生成”
4. 节点重名时会自动生成唯一名称，例如 `美国1`、`美国1 #2`，固定入口需绑定这些最终名称
5. 导出的 Clash YAML 会在 `listeners` 中写入端口、节点、用户名和密码
